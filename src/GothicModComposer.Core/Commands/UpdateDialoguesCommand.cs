...
using GothicModComposer.Core.Utils.Daedalus;
    ...
    public class UpdateDialoguesCommand : ICommand
    {
        ...
        public async Task ExecuteAsync()
        {
            if (!_profile.CommandsConditions.UpdateDialoguesStepRequired)
            {
                ...
                {
                    Logger.Warn(
                        "Update dialogues is not required, but OU.CSL file does not exist so this step is needed.");
                }
                else
                {
        ...
        private static List<Tuple<string, string>> ReadDialoguesFromScripts(List<string> scriptPaths)
        {
            ...
            using (var progress =
                new ProgressBar(scriptPaths.Count, "Updating dialogues", ProgressBarOptionsHelper.Get()))
            {
                var counter = 1;
        ...
        private static void GenerateOuCslFile(string ouCslPath, List<Tuple<string, string>> dialoguePopupsRecords)
        {
            ...
            File.WriteAllText(ouCslPath, CslWriter.GenerateContent(dialoguePopupsRecords),
                EncodingHelper.GothicEncoding);
